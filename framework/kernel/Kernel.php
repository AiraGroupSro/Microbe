...
namespace AiraGroupSro\Microbe\framework\kernel;

use AiraGroupSro\Microbe\framework\router\Exception\AbstractException as RouterException;
use AiraGroupSro\Microbe\framework\traverser\Traverser;
...
class Kernel{
	...
	protected function determineRootUri(){
		return 'http'.(true === isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 's' : '').'://'.$_SERVER['HTTP_HOST'].$this->determineBaseUri();
	}
